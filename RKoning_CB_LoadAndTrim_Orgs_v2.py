...
#       enumerate date columns by years, inclusive
enum_yr = [2015, 2008, 2000, 1980, 1945, 1900]
...
def CB_specialColType_date(dat: str) -> str:
    ...
    if len(dat) < 6:
        ...
        # Assume correct
       if I_yr_cutoff:
            if int( dat[::-1].split('/',1)[0][::-1].replace(" ","") ) < yr_cutoff:
...
def CB_enumerateColumn(datCol: pandas.core.frame.DataFrame, colType: str) -> list:
    if colType == "date":
        ...
        enum_r = range(0,len(enum_yr)-1)

        iRow = 0
        for datRow in datCol:
            if datRow is not None:

                if datRow.year > enum_yr[0]:
                    datEnum[iRow] = 0
                elif datRow.year < enum_yr[-1]:
                    datEnum[iRow] = len(enum_yr)
                for iEnum in enum_r:
                    if datRow.year <= enum_yr[iEnum] and datRow.year >= enum_yr[iEnum+1]:
                        datEnum[iRow] = iEnum+1
...
def main():
    ...
    if I_flag:
        ...
        # Convert date columns to data type = datetime
        for colName in list(colType_json):
            if colType_json[colName]=='date':
                if colName in dat.columns:
                    print("Converting column " + colName + " to datetime")
                    dat[colName] = pandas.to_datetime(dat[colName])
